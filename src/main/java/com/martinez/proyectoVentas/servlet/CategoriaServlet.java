...
public class CategoriaServlet extends HttpServlet { 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			com.martinez.proyectoVentas.bean.CategoriaBean dato = new com.martinez.proyectoVentas.bean.CategoriaBean();
			if ("l".equalsIgnoreCase(accion)) {
				com.martinez.proyectoVentas.dao.CategoriaDao dao = new com.martinez.proyectoVentas.dao.CategoriaDaoImpl();
				String filtro = "";
				String lbBusqueda = request.getParameter("lbBusqueda");
				if(null!=lbBusqueda && !lbBusqueda.isEmpty()) {
					filtro = " where NombreCate like ('%"+lbBusqueda+"%')";
				}
				filtro += " order by NombreCate asc";
				ArrayList<com.martinez.proyectoVentas.bean.CategoriaBean> lista = dao.select(filtro);
				String base = "<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td>"+ "<td><button type='button' class='btn btn-info px-3' onClick='return Clickbtn_Editar(%s);' ><i class='fa fa-search-plus' aria-hidden='true'></i></button>"
				...
				if (lista != null && !lista.isEmpty()) {
					...
					int i = 0;
					while (i < lista.size()) {
						com.martinez.proyectoVentas.bean.CategoriaBean idato = lista.get(i);
						html += String.format(base, idato.getCodigo(),idato.getNombre(),idato.getDescripcion(),idato.getDetalle(),idato.getCodigo(),idato.getCodigo());
						i++;
					}
					res.setHtml(html);
				}
				...
				com.martinez.proyectoVentas.dao.CategoriaDao dao = new com.martinez.proyectoVentas.dao.CategoriaDaoImpl();
				dato.setCodigo(Integer.parseInt(request.getParameter("codigo")));
				com.martinez.proyectoVentas.bean.CategoriaBean rest = dao.selectOne(dato);
				res.setValor(rest);
			} else {
				res.setCodigo("2002");
